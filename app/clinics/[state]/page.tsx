...
export default async function ClinicsPage({ searchParams }: { searchParams?: any }) {
  ...
  let eQ = supabase
    .from('events')
    .select('id,name,location,state,start_date,end_date,website,signup_url,cover_url,verified,slug')
    .eq('kind','clinic')
    .gte('start_date', today)
  if (q)     eQ = eQ.or(`name.ilike.%${q}%,location.ilike.%${q}%`)
  if (state) eQ = eQ.eq('state', state)

  const { data: clinics } = await eQ.order('start_date', { ascending: true })
  ...
  return (
    <section className="grid gap-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
        ...
        {(!clinics || clinics.length===0) && <div className="card"><p className="text-sm text-gray-700">No clinics found.</p></div>}
      </div>
    </section>
  )
}
