...
export default async function LeaguesPage({ searchParams }: { searchParams?: any }) {
  ...
  return (
    <section className="container py-8">
      <Breadcrumbs items={[{ label: 'Leagues' }]} className="mb-6" />

      <div className="space-y-3">
        <h1 className="section-heading">Verified league finder</h1>
      ...
      <div className="surface-card mt-8 mb-8">
        <form className="grid gap-5 p-6 sm:p-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select className="select" name="contact" defaultValue={contact}>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {league.divisions?.slice(0, 3).map((div: string) => (
            <span key={div} className="px-2.5 py-1 text-xs font-medium rounded-lg bg-gray-50 text-[#345c72] border border-gray-100">
              {div}
            </span>
          ))}
          {league.formats?.map((f: string) => (
            <span key={f} className="px-2.5 py-1 text-xs font-medium rounded-lg bg-gray-50 text-[#345c72] border border-gray-100">
              {f}
            </span>
          ))}
        </div>
